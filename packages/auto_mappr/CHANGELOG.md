...
## 2.17.0
- Fix mapping into constructor parameters that match a source field but not a field of the target class, such as `Target.foo({required int a}) : secret = a;`. [#238](https://github.com/netglade/auto_mappr/issues/238)
- The reverse mapping generated by `reverse` no longer uses the target's constructor. Use the new `sourceConstructor` on `MapType` to select one for it. [#238](https://github.com/netglade/auto_mappr/issues/238)
- Support `targetConstructor` on `MapType`, the new name of the now deprecated `constructor`. [#238](https://github.com/netglade/auto_mappr/issues/238)
- Fail the build when `sourceConstructor` is set without `reverse`, or when both `constructor` and `targetConstructor` are set. [#238](https://github.com/netglade/auto_mappr/issues/238)
