...
class AutoMapprGenerator extends GeneratorForAnnotation<annotation.AutoMappr> {
  ...
  static const String mapTypeFieldTargetConstructor = 'targetConstructor';
  static const String mapTypeFieldSourceConstructor = 'sourceConstructor';
  ...
  List<TypeMapping> _processMappers({
    ...
    final res = mappers.map((mapper) {
      ...
      final targetConstructor = mapper.getField(mapTypeFieldTargetConstructor)?.toStringValue();
      final sourceConstructor = mapper.getField(mapTypeFieldSourceConstructor)?.toStringValue();
      ...
      // `constructor` is the deprecated name of `targetConstructor`.
      if (constructor != null && targetConstructor != null) {
        final emittedSource = EmitterHelper.current.typeReferEmitted(type: sourceType);
        final emittedTarget = EmitterHelper.current.typeReferEmitted(type: targetType);

        throw InvalidGenerationSourceError(
          "MapType<$emittedSource, $emittedTarget> sets both the deprecated 'constructor' ('$constructor') and 'targetConstructor' ('$targetConstructor'), which select the same thing.",
          element: element,
          todo: "Remove 'constructor' and keep 'targetConstructor'.",
        );
      }

      // `sourceConstructor` only configures the mapping generated by `reverse`,
      // so setting it without `reverse` has no effect and is most likely a mistake.
      if (sourceConstructor != null && !(isReverse ?? false)) {
        final emittedSource = EmitterHelper.current.typeReferEmitted(type: sourceType);
        final emittedTarget = EmitterHelper.current.typeReferEmitted(type: targetType);

        throw InvalidGenerationSourceError(
          "MapType<$emittedSource, $emittedTarget> selects the source constructor '$sourceConstructor' but does not set 'reverse', so the mapping using it is never generated.",
          element: element,
          todo: "Set 'reverse' to true, or use 'targetConstructor' to select the constructor of $emittedTarget.",
        );
      }
      ...
      return [
        ...
        if (isReverse ?? false)
          TypeMapping(
            ...
            typeConverters: [..._toTypeConverters(mapTypeConverters), ...globalConverters],
            whenSourceIsNullExpression: whenSourceIsNull,
            // `targetConstructor` selects a constructor of the target,
            // which the reverse mapping maps *from*. Use `sourceConstructor` instead.
  ...
  Iterable<DartObject> _convertersFromRecursiveIncludes({required List<DartObject> includesList}) {
    ...
    for (final include in includesList) {
      final x = include.type?.element;
      if (x is! InstanceElement) {
        continue;
      }
